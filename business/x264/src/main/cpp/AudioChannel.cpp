...
void AudioChannel::openCodec(int sampleRate, int channels) {
    /**
    unsigned long   sampleRate,        // 采样率，单位是bps
    unsigned long   channels,          // 声道，1为单声道，2为双声道
    unsigned long   &inputSamples,     // 传引用，得到每次调用编码时所应接收的原始数据长度
    unsigned long   &maxOutputBytes    // 传引用，得到每次调用编码时生成的AAC数据的最大长度
     */
